...
export async function POST(_request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!hasPermission(session.roleName, "payroll", "void")) {
    ...
    const result = await payrollService.voidPayRun(schema, session.userId, (await params).id)
    return NextResponse.json(formatApiResponse(result))
  } catch (error) {
    console.error("Pay run void error:", error)
    return NextResponse.json(formatApiError("ERR_INTERNAL", error instanceof Error ? error.message : "Failed to void pay run"), { status: 500 })
  }
}
