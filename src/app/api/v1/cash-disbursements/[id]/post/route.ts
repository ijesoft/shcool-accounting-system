...
export async function POST(_request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!hasPermission(session.roleName, "cash_disbursements", "post")) {
    ...
    const result = await cashDisbursementsService.post(schema, session.userId, id)
    return NextResponse.json(formatApiResponse(result))
  } catch (error: any) {
    console.error("Disbursement post error:", error)
