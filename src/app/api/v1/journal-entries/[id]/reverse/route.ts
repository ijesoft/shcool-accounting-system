...
export async function POST(_request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const entry = await journalEntryService.reverse(schema, id, session.userId)
    ...
    return NextResponse.json(formatApiError("ERR_INTERNAL", "Failed to reverse entry"), { status: 500 })
  }
}
