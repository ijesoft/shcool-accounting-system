...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const entry = await journalEntryService.approve(schema, id, session.userId, body.comments)
    ...
    return NextResponse.json(formatApiError("ERR_INTERNAL", "Failed to approve entry"), { status: 500 })
  }
}
