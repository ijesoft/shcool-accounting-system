...
export async function POST(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    await fiscalCalendarService.closePeriod(schema, id, session.userId)
    return NextResponse.json(formatApiResponse({ closed: true }))
  } catch (error: any) {
    const message = error?.message || "Failed to close fiscal period"
