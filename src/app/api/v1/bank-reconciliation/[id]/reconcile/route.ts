...
export async function POST(_request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!hasPermission(session.roleName, "bank_reconciliation", "post")) {
    ...
    console.error("Bank reconciliation reconcile error:", error)
    return NextResponse.json(formatApiError("ERR_INTERNAL", "Failed to reconcile"), { status: 500 })
  }
}
