...
export async function GET(request: NextRequest) {
  try {
    ...
    const result = await reportService.getChangesInEquity(schema, from, to)
    return NextResponse.json(formatApiResponse(result))
  } catch (error) {
    console.error("Statement of changes in equity error:", error)
    return NextResponse.json(formatApiError("ERR_INTERNAL", "Failed to generate statement of changes in equity"), { status: 500 })
  }
}
