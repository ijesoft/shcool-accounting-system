...
export async function GET(request: NextRequest) {
  try {
    ...
    const fiscalPeriodId = request.nextUrl.searchParams.get("period") || undefined
    const result = await reportService.getTrialBalance(schema, fiscalPeriodId)
    return NextResponse.json(formatApiResponse(result))
  } catch (error) {
    console.error("Trial balance error:", error)
    return NextResponse.json(formatApiError("ERR_INTERNAL", "Failed to generate trial balance"), { status: 500 })
  }
}
