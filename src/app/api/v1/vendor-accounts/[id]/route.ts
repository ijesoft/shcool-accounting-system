...
export async function GET(_request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const result = await vendorAccountService.getById(schema, id)
    if (!result) return NextResponse.json(formatApiError("ERR_NOT_FOUND", "Vendor not found"), { status: 404 })
    return NextResponse.json(formatApiResponse(result))
  } catch (error) {
    console.error("Vendor get error:", error)
    return NextResponse.json(formatApiError("ERR_INTERNAL", "Failed to get vendor"), { status: 500 })
  }
}
