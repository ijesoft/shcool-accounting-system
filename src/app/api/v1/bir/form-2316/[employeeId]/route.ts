...
export async function GET(
  ...
  try {
    ...
    if (isNaN(year)) {
      return NextResponse.json(formatApiError("ERR_VALIDATION", "Invalid year parameter"), { status: 400 })
    }

    const data = await form2316Service.generate(schema, session.entityId, employeeId, year)

    if (format === "html") {
      const html = form2316Service.buildHtmlTemplate(data)
    ...
    console.error("Form 2316 error:", error)
