...
import { slspService } from "@/lib/bir/slsp"
...
export async function GET(request: NextRequest) {
  try {
    ...
    const [sales, purchases] = await Promise.all([
      slspService.generateSales(schema, session.entityId, from, to),
      slspService.generatePurchases(schema, session.entityId, from, to),
    ])

    if (format === "csv") {
      const csv = slspService.toCsv(sales, purchases)
      return new NextResponse(csv, {
        headers: {
          "Content-Type": "text/csv; charset=utf-8",
          "Content-Disposition": `attachment; filename="SLSP_${from}_${to}.csv"`,
        },
      })
    }

    return NextResponse.json(formatApiResponse({ sales, purchases }))
  } catch (error) {
    console.error("SLSP error:", error)
    return NextResponse.json(
      formatApiError("ERR_INTERNAL", error instanceof Error ? error.message : "Failed to generate SLSP"),
      { status: 500 }
    )
  }
}
