  ...
  async generate(entitySchema: string, from: string, to: string): Promise<STOFSummary> {
    const rows = await prisma.$queryRawUnsafe<any[]>(
      `SELECT
         ...
         COALESCE(si.semester, '') as semester,
         ...
         COALESCE(SUM(si.balance_due), 0) as balance_due
       FROM "${entitySchema}".student_invoice si
       LEFT JOIN "${entitySchema}".student s ON s.id = si.student_id
       LEFT JOIN "${entitySchema}".student_invoice_line sil ON sil.invoice_id = si.id
       WHERE si.invoice_date >= $1::date AND si.invoice_date <= $2::date
       GROUP BY si.student_id, s.full_name, s.grade_level, si.semester
       ORDER BY student_name, si.semester`,
      from, to
    ).catch(() => [] as any[])
